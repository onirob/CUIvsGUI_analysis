...
# ============================================================
# H4a ALTERNATIVE
# Task complexity -> Mental Workload
#
...
# Alternative H4a:
#   Higher task complexity is associated with higher MWL
#   AFTER controlling for industry_role and interface condition.
#
...
OUT_DIR  <- file.path(SCRIPT_DIR, "results_H4_ALTERNATIVE")
...
save_csv(df, alt_file("h4a_cleaned_analysis_data.csv"))
...
save_csv(role_counts, alt_file("h4a_industry_role_counts.csv"))
save_csv(role_by_interface, alt_file("h4a_industry_role_by_interface.csv"))
save_csv(desc_raw_by_role, alt_file("h4a_descriptives_raw_by_industry_role.csv"))
save_csv(industry_role_omnibus, alt_file("h4a_industry_role_omnibus.csv"))
...
save_csv(desc_raw_by_complexity, alt_file("h4a_descriptives_raw_by_complexity.csv"))
save_csv(desc_raw_by_cell, alt_file("h4a_descriptives_raw_by_cell.csv"))
save_csv(desc_ord_by_complexity, alt_file("h4a_descriptives_ordinal_by_complexity.csv"))
save_csv(desc_ord_by_cell, alt_file("h4a_descriptives_ordinal_by_cell.csv"))
save_csv(n_participants_by_interface, alt_file("h4a_n_participants_by_interface.csv"))
...
capture.output(summary(m_null), file = alt_file("h4a_null_model_summary.txt"))
capture.output(summary(m_add), file = alt_file("h4a_additive_model_summary.txt"))

# ============================================================
# MODEL FIT / LRT
# ============================================================
fit_tbl <- tibble(
  model  = c("null_interface_role", "additive_complexity_adjusted_for_industry_role"),
...
save_csv(fit_tbl, alt_file("h4a_model_fit.csv"))

lrt_tbl <- as.data.frame(anova(m_null, m_add))
save_csv(lrt_tbl, alt_file("h4a_complexity_lrt.csv"))
capture.output(anova(m_null, m_add), file = alt_file("h4a_complexity_lrt.txt"))
...
coef_add_industry_role_terms_only <- coef_add_location %>%
  filter(str_detect(term, INDUSTRY_ROLE_COL))

save_csv(coef_null_all, alt_file("h4a_null_coef_all.csv"))
save_csv(coef_add_all, alt_file("h4a_additive_coef_all.csv"))
save_csv(coef_null_location, alt_file("h4a_null_coef_location.csv"))
save_csv(coef_add_location, alt_file("h4a_additive_coef_location.csv"))
save_csv(coef_add_industry_role_terms_only, alt_file("h4a_additive_industry_role_terms_only.csv"))

# ============================================================
# RANDOM EFFECT SD
# ============================================================
rand_null <- extract_random_effect_sd(m_null, "null_interface_role")
rand_add  <- extract_random_effect_sd(m_add, "additive_complexity_adjusted")
rand_tbl  <- bind_rows(rand_null, rand_add)

save_csv(rand_tbl, alt_file("h4a_random_effect_sd.csv"))
...
save_csv(
  ...
  alt_file("h4a_emm_complexity_latent.csv")
)

save_csv(
  contrasts_complexity_latent,
  alt_file("h4a_contrasts_complexity_latent.csv")
)

save_csv(
  summary(emm_complexity_meanclass, level = CONF_LEVEL) %>% standardize_emm_ci_cols(),
  alt_file("h4a_emm_complexity_meanclass.csv")
)

save_csv(
  contrasts_complexity_meanclass,
  alt_file("h4a_contrasts_complexity_meanclass.csv")
)
...
save_csv(
  ...
  alt_file("h4a_emm_interface_latent.csv")
)

save_csv(
  overall_interface_contrast_latent,
  alt_file("h4a_overall_interface_contrast_latent_from_additive_model.csv")
)
...
save_csv(prob_cells_tbl, alt_file("h4a_category_probabilities_by_cell.csv"))

# ============================================================
# FINAL CONSOLE NOTES
# ============================================================
cat("\n============================================================\n")
cat("H4a ALTERNATIVE completata: adjusted model for industry_role.\n")
cat("Output salvati in:", OUT_DIR, "\n")
cat("All the file have suffisso _ALTERNATIVE.\n")
cat("============================================================\n\n")

cat("Model nullo adjusted:\n")
cat("y_ord ~ industry_role + interface_cond + (1|participant_id)\n\n")

cat("Additive model adjusted:\n")
cat("y_ord ~ industry_role + interface_cond + task_complexity + (1|participant_id)\n\n")

cat("Come cambia H4a:\n")
cat("- Before: task_complexity prediceva MWL controlling for interface_cond.\n")
cat("- Ora: task_complexity must predire MWL AFTER control for interface_cond and industry_role.\n")
...
cat("- h4a_complexity_lrt_ALTERNATIVE.csv\n")
cat("- h4a_additive_coef_location_ALTERNATIVE.csv\n")
cat("- h4a_additive_industry_role_terms_only_ALTERNATIVE.csv\n")
cat("- h4a_contrasts_complexity_latent_ALTERNATIVE.csv\n")
cat("- h4a_contrasts_complexity_meanclass_ALTERNATIVE.csv\n")
cat("- h4a_industry_role_omnibus_ALTERNATIVE.csv\n")
cat("- h4a_category_probabilities_by_cell_ALTERNATIVE.csv\n")